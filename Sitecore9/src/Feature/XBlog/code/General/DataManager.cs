...
namespace Sitecore.Feature.XBlog.General
{
    public class DataManager
    {
        ...
        {
            return GetBlogHomeItem(Context.Item);
        }
        ...
        {
            return GetBlogSettingsItem(Context.Item);
        }
        public static BlogSettings GetBlogSettingsItem(Item item)
        {
            var blogHomeItem = GetBlogHomeItem(item);
            BlogHome homeItem = GetBlogHomeItem(item).CreateAs<BlogHome>();
            if (homeItem == null)
                return null;
            else
            {
                var blogSettings = homeItem.BlogSettings.CreateAs<BlogSettings>();
                return blogSettings;
            }
