...
using XBlogHelper.Models.Import;

namespace XBlogHelper.Import
{
    public class BlogCreator
    {
        public static bool CreateBlog(string blogName, string blogType, string importFileName, string blogStartId)
        {
            try
            {
                ...
                {
                    hasImportFile = true;
                    fileLocation = Sitecore.Configuration.Settings.DataFolder.ToString() + @"\" + importFileName;
                }

                string creationPath = Sitecore.Context.Database.GetItem(blogStartId).Paths.FullPath;
